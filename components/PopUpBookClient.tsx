...
// How much a clicked paragraph enlarges inside its popup, and the popup's
// fixed on-screen size. Zoomed gently on purpose — a comfortable reading
// window (more words visible at once), not a tight pixel-peeping crop.
const POPUP_ZOOM = 2.2;
const POPUP_W = 720;
const POPUP_H = 620;

/**
 * A plain page image — the fit-to-screen page. Clicking anywhere on it
 * opens a <ParagraphPopup> zoomed in on that spot (see onOpenPopup below),
 * standing in for "click a paragraph to read it larger" on scans that
 * don't carry separate per-paragraph text.
 */
function PagePane({
  src,
  alt,
  ...
  src: string;
  alt: string;
  onOpenPopup?: (e: React.MouseEvent<HTMLDivElement>) => void;
}) {
  return (
    <div
      onClick={onOpenPopup}
      style={{ position: "absolute", inset: 0, cursor: onOpenPopup ? "pointer" : "default" }}
    >
...
/**
 * A single "paragraph" reading popup — a fixed-size zoomed-in crop of
 * whichever page was clicked, centered on the click point (clamped so it
 * never shows past the page's own edges). Dismissed by its own close
...
function ParagraphPopup({
  ...
  const zoomedW = pageWidth * POPUP_ZOOM;
  const zoomedH = pageHeight * POPUP_ZOOM;
  // Center the crop on the click point, then clamp so the popup never
  // reveals blank space past the real page edges.
  const rawX = clickX * POPUP_ZOOM - POPUP_W / 2;
  const rawY = clickY * POPUP_ZOOM - POPUP_H / 2;
  const maxX = Math.max(0, zoomedW - POPUP_W);
  const maxY = Math.max(0, zoomedH - POPUP_H);
  const offsetX = Math.min(Math.max(rawX, 0), maxX);
  const offsetY = Math.min(Math.max(rawY, 0), maxY);

  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label="Enlarged paragraph"
      onClick={onClose}
      style={{
        position: "fixed",
        inset: 0,
    ...
    >
      ...
      >
        ...
        >
          <Image src={src} alt={alt} fill sizes="1400px" style={{ objectFit: "contain" }} />
        </div>
        <button
          onClick={onClose}
...
/* ─────────────────────────────────────────────
   MAIN COMPONENT
───────────────────────────────────────────── */
export default function PopUpBookClient() {
  ...
  // ── Click-to-enlarge paragraph popup (full-screen reader) ───────────────
  // Scanned pages don't carry separate per-paragraph text, so "click a
  // paragraph" is approximated as "click anywhere on the page" — the popup
  // zooms in on that exact spot, clamped to the page's own edges.
  ...
  function openPopup(e: React.MouseEvent<HTMLDivElement>, src: string, alt: string) {
    ...
    <main style={{ background: C.bg, minHeight: "100vh", color: C.heading }}>
      ...
      {fullscreenReader && (
        ...
        >
          ...
          >
            ...
            <div style={{ flex: 1, background: C.bookPage, position: "relative", overflow: "hidden" }}>
              <PagePane
                ...
                onOpenPopup={(e) => openPopup(e, `/images/pdf-pages/page-${String(pdfPage).padStart(2, "0")}.png`, `Book page ${pdfPage}, enlarged`)}
              />
            ...
            <div style={{ flex: 1, background: C.bookPage, position: "relative", overflow: "hidden" }}>
              {pdfPage + 1 <= 62 ? (
                <PagePane
                  ...
                  onOpenPopup={(e) => openPopup(e, `/images/pdf-pages/page-${String(pdfPage + 1).padStart(2, "0")}.png`, `Book page ${pdfPage + 1}, enlarged`)}
                />
              ) : (
          ...
          >
            Pages {pdfPage}–{Math.min(pdfPage + 1, 62)} of 62 · Click a paragraph to enlarge · ← → to turn pages · Esc to close
          </p>
        </div>
      )}
